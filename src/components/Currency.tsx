...
export function Currency({ value, className, symbolClassName, amountClassName, style, ...props }: CurrencyProps) {
  const hidden = useFinancas(s => s.valuesHidden)
  const display = useAnimatedValue(Math.abs(value), hidden)
  const negative = value < 0
  const amountStyle = style?.color ? { color: style.color } : undefined
  return <span className={cn('number inline-flex items-baseline whitespace-nowrap',className)} style={style} {...props}>
    ...
    {hidden
      ...
      :<NumberFlow value={display} locales="pt-BR" format={{minimumFractionDigits:2,maximumFractionDigits:2}} className={amountClassName} style={amountStyle}/>}
  </span>
}
