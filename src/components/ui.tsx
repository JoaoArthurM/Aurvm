...
export const EyeToggle = ({ className }: { className?: string }) => {
  ...
  return <button type="button" aria-label={hidden ? 'Mostrar valores' : 'Ocultar valores'} onClick={toggle} className={cn('grid h-7 w-7 shrink-0 place-items-center rounded-full border border-border bg-surface text-t3 transition active:scale-95', className)}>{hidden ? <IconEyeOff size={14}/> : <IconEyeCheck size={14}/>}</button>
}
