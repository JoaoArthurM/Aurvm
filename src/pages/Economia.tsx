...
import { IconArrowRight, IconChevronDown, IconPencil, IconStarFilled, IconTrash as Trash2 } from '@tabler/icons-react'
...
function EconomyRow({item}:{item:EconomiaItem}) {
  ...
  return <Card className="rounded-[18px] p-3.5">
    ...
    <div className="mt-3 flex items-center gap-1.5">
      ...
      {item.tipo==='pontual'&&<label className={`${chip} min-w-0 gap-1 px-3`}><IconStarFilled aria-hidden="true" size={9} className="shrink-0 text-yellow"/><input type="month" value={item.mes??''} onChange={e=>update(x=>{x.mes=e.target.value})} className="min-w-0 bg-transparent outline-none"/></label>}
