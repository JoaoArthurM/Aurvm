...
import {
  ...
  IconChevronDown, IconChevronLeft as ChevronLeft, IconChevronRight as ChevronRight, IconCreditCard,
  IconEye, IconEyeOff, IconPigMoney, IconPlus as Plus, IconReceiptDollar, IconSearch as Search,
  IconSortAscendingLetters, IconSortDescendingNumbers, IconTag, IconTrash, IconX as X,
} from '@tabler/icons-react'
import { AddButton, Button, Card, Input, MoneyInput } from '../components/ui'
import { Currency } from '../components/Currency'
import { cn, fluxMeta, getLimites, money, monthKey, saldoStyle, tempTiers, uid } from '../lib/utils'
...
function Saldos({month}:{month:Date}){
 ...
 const nextNet=data.flux.lancamentos.filter(l=>l.data.startsWith(monthKey(nextDate))).reduce((a,l)=>a+(l.tipo==='entrada'?l.valor:-l.valor),0)
 ...
 const cols='grid-cols-[minmax(0,1fr)_26px_76px_26px_76px]'
 return <Card className="overflow-hidden rounded-none border-x-0">
  <div className={cn('grid items-center border-b border-border bg-el/60 py-2 text-[9px] font-bold uppercase tracking-wider text-t3',cols)}>
   <label className="relative ml-2 inline-flex h-7 w-max items-center gap-1.5 rounded-full border border-border bg-surface pl-1.5 pr-6">
    <span className="grid h-4 w-4 place-items-center rounded-full" style={filter==='total'?{color:'var(--t2)',background:'var(--el)'}:{color:fluxMeta[filter].color,background:`${fluxMeta[filter].color}18`}}>{filter==='total'?<i className="h-1.5 w-1.5 rounded-full bg-t2"/>:<FluxTypeIcon tipo={filter} size={10}/>}</span>
    <select aria-label="Filtrar transações por tipo" value={filter} onChange={e=>setFilter(e.target.value as SaldoFilter)} className="appearance-none bg-transparent text-[9px] font-bold uppercase tracking-wider text-t1 outline-none">
     <option value="total">Total</option>
     {(Object.keys(fluxMeta) as FluxTipo[]).map(tipo=><option key={tipo} value={tipo}>{fluxMeta[tipo].label}</option>)}
    </select>
    <IconChevronDown size={11} className="pointer-events-none absolute right-2 text-t3"/>
   </label>
  ...
  {rows.map(row=>{
   ...
   const DayCell=({date,exists=true}:{date:Date;exists?:boolean})=><span className={cn('number flex items-center justify-center border-l border-border/60 bg-el/40 text-[10px] font-bold',!exists&&'opacity-0',isSunday(date,row.day)?'text-red':'text-t2',row.day===nowDay&&'text-flux')}>{String(row.day).padStart(2,'0')}</span>
   return <div key={row.date} className="border-b border-border/60 last:border-0">
    ...
    {open&&row.tx.length>0&&<div className="space-y-1.5 border-t border-border/60 bg-el/40 px-3 py-2.5">
     {row.tx.map(tx=><div key={tx.id} className="flex items-center gap-2">
      <span className="grid h-5 w-5 shrink-0 place-items-center rounded-md" style={{color:fluxMeta[tx.tipo].color,background:`${fluxMeta[tx.tipo].color}18`}}><FluxTypeIcon tipo={tx.tipo} size={11}/></span>
      <span className="min-w-0 flex-1 truncate text-[10px] text-t2">{tx.descricao}</span>
      <Currency value={tx.valor} className="text-[10px] font-bold" style={{color:fluxMeta[tx.tipo].color}}/>
     </div>)}
    </div>}
   </div>
  })}
...
function Tags({monthKey:key}:{monthKey:string}){
 ...
 const totalOf=(id:string)=>data.flux.lancamentos.filter(l=>l.data.startsWith(key)&&l.tag_id===id).reduce((a,b)=>a+b.valor,0)
...
function Horizon({close}:{close:()=>void}){
 ...
 const months=Array.from({length:6},(_,i)=>new Date(2026,6+i,1)).map(start=>{
  ...
  const dias=Array.from({length:total},(_,i)=>{
   ...
   data.flux.lancamentos.filter(l=>l.data===date).forEach(l=>{running+=l.tipo==='entrada'?l.valor:-l.valor})
   return{day,saldo:running,domingo:new Date(start.getFullYear(),start.getMonth(),day).getDay()===0}
  })
  return{start,dias}
 })
 return <div className="absolute inset-0 z-50 overflow-auto bg-bg">
  ...
  <div className="flex w-max gap-2 p-4">
   {months.map(({start,dias})=><div key={String(start)} className="w-[118px]">
    <p className="mb-1.5 text-center text-[9px] font-bold uppercase tracking-wider text-t3">{new Intl.DateTimeFormat('pt-BR',{month:'short',year:'2-digit'}).format(start).replace('.','')}</p>
    <div className="overflow-hidden rounded-[10px] border border-border">
     {dias.map(({day,saldo,domingo})=><div key={day} className="grid grid-cols-[24px_1fr] border-b border-border/60 last:border-0">
      <span className={cn('number flex items-center justify-center bg-el/50 py-[7px] text-[9px] font-bold',domingo?'text-red':'text-t2')}>{String(day).padStart(2,'0')}</span>
      <span className="flex items-center justify-end pr-1.5 text-[9px] font-semibold" style={saldoStyle(saldo,limites)}>{hidden?'••••':money(saldo)}</span>
     </div>)}
    </div>
   </div>)}
  </div>
...
function NewTransaction({close}:{close:()=>void}){const {data,mutate}=useFinancas();const [type,setType]=useState<FluxTipo>('saida');const [desc,setDesc]=useState('');const [value,setValue]=useState(0);const [tag,setTag]=useState('');const cartoes:Cartao[]=data.flux.cartoes??[];return <div className="absolute inset-0 z-50 flex items-end bg-black/70"><div className="safe-bottom w-full rounded-t-[28px] border-t border-border bg-surface p-5"><div className="mb-5 flex items-center justify-between"><h2 className="text-lg font-bold">Nova movimentação</h2><button onClick={close} className="p-2 text-t3"><X size={18}/></button></div><div className="mb-4 flex gap-2 overflow-x-auto">{(Object.keys(fluxMeta) as FluxTipo[]).map(x=><button key={x} onClick={()=>setType(x)} className="flex shrink-0 items-center gap-1.5 rounded-full border px-3 py-2 text-[9px] font-semibold" style={{borderColor:type===x?fluxMeta[x].color:'var(--border)',color:type===x?fluxMeta[x].color:'var(--t2)'}}><FluxTypeIcon tipo={x} size={11}/>{fluxMeta[x].label}</button>)}</div>{type==='cartao'&&cartoes.length>0&&<p className="mb-3 flex items-center gap-1.5 text-[9px] text-t3"><IconCreditCard size={12} className="shrink-0 text-flux"/>Próximo vencimento: {cartoes.map(c=>`${c.nome} · ${nextDueLabel(c.vencimento)}`).join('  ·  ')}</p>}<div className="space-y-2"><Input placeholder="Descrição" value={desc} onChange={e=>setDesc(e.target.value)} className="focus:border-flux"/><MoneyInput aria-label="Valor" value={value} onValueChange={setValue} className="focus-within:border-flux"/><select value={tag} onChange={e=>setTag(e.target.value)} className="h-11 w-full rounded-xl border border-border bg-el px-3 text-sm text-t2 outline-none"><option value="">Sem tag</option>{data.flux.tags.filter(t=>!t.oculta).map(t=><option key={t.id} value={t.id}>{t.label}</option>)}</select></div><button disabled={!desc||!value} onClick={()=>{mutate(d=>d.flux.lancamentos.push({id:uid(),data:new Date().toISOString().slice(0,10),tipo:type,valor:value,descricao:desc,tag_id:tag||null}));close()}} className="mt-4 h-12 w-full rounded-xl bg-flux text-sm font-bold text-white disabled:opacity-40">Adicionar</button></div></div>}
